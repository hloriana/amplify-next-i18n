...
describe('LP Test /de/smlp/high-school-abroad/', () => {

  it('Form successfully submits', () => {

    cy.intercept('GET', /ef.com\/wwen\/forms/).as('getForm')

    cy.visit('/de/smlp/high-school-abroad/')
    ...
    cy.get(`#${formType}-productRadio-IA`)
      .check({force: true})
    ...
    // expect(response?.body?.extendedDetail?.ProductCode).to.equal('IA');
    // expect(response?.body?.extendedDetail?.ProgramCode).to.equal('IB');
