...
const lernaRoot = lernaConfig !== null ? path.dirname(lernaConfig.filepath) : null;
export default function loader(content, inputSourceMap) {
  ...
  const outputFilename = normalize(path.join(path.isAbsolute(cacheDirectory) ? cacheDirectory : path.join(process.cwd(), cacheDirectory), this.resourcePath.includes(root) ? path.relative(root, baseOutputFileName) : baseOutputFileName));
  const resolveOptions = {
    extensions: ['.js', '.jsx', '.ts', '.tsx', '.json']
  };
  const resolveSync = enhancedResolve.create.sync( // this._compilation is a deprecated API
  ...
  this._compilation?.options.resolve ? { ...resolveOptions,
  ...
  if (result.cssText) {
    ...
    this.callback(null, `${result.code}\n\nrequire(${loaderUtils.stringifyRequest(this, outputFilename)});`, result.sourceMap ?? undefined);
    return;
  }
