      ...
      case 'var':
        {
          ...
          if (binding.path.isVariableDeclarator() && t.isSequenceExpression(binding.path.node.init)) {
            // Replace SequenceExpressions (expr1, expr2, expr3, ...) with the last one
            decl = t.variableDeclarator(binding.path.node.id, binding.path.node.init.expressions[binding.path.node.init.expressions.length - 1]);
          }
