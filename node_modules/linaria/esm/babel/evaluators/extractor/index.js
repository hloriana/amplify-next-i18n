...
const extractor = (filename, options, text, only = null) => {
  ...
  let {
    code
  } = transformSync(text, transformOptions);
    ...
    Program(path) {
      ...
      for (let idx = body.length - 1; idx >= 0; idx--) {
        if (isLinariaPrevalExport(body[idx])) {
          // Here we are!
          const statements = RequirementsResolver.resolve(body[idx].get('expression.right')); // We only need to evaluate the last item in a sequence expression, e.g. (a, b, c)
          ...
          const wrapped = statements.reduce((acc, curr) => t.blockStatement([curr, acc]), t.blockStatement([body[idx].node])); // Generate a new code with extracted statements

          code = [// Use String.raw to preserve escapes such as '\n' in the code
          String.raw`${generator(wrapped).code}`].join('\n');
          break;
        }
      }
